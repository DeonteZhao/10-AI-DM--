...
export const LOCAL_BETA_ACCESS_BYPASS_HEADER = "x-dice-tales-beta-local-bypass";

const PUBLIC_PREVIEW_PATH_PREFIXES = ["/preview", "/mock", "/demo"];
...
export function isProtectedPlayerPath(pathname: string) {
  ...
  return !PUBLIC_PREVIEW_PATH_PREFIXES.some((prefix) => pathname === prefix || pathname.startsWith(`${prefix}/`));
}
...
export function isLocalBetaAccessBypassed() {
