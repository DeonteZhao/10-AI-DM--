...
class Task4RulesAndAiContextTest(unittest.TestCase):
  ...
  def create_runtime(self) -> tuple[str, dict[str, str]]:
    scenario_id = "task4_case"
    headers = self.auth_headers("task4_runtime@example.com")
    ...
    return session_response.json()["session"]["id"], headers

  def test_session_check_endpoint_uses_unified_coc_rule_result(self):
    session_id, headers = self.create_runtime()
