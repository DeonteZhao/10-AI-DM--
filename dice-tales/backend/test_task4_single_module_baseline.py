...
class Task4SingleModuleBaselineTest(unittest.TestCase):
  ...
  def test_docx_module_works_in_public_admin_and_runtime_flows(self):
    ...
    self.assertGreaterEqual(len(structured_payload["locations"]), 1)
    self.assertGreaterEqual(len(structured_payload["sequence"]), 1)
    ...
    session_response = self.client.post(
      "/sessions",
      json={"scenario_id": seed.MODULE_ID, "investigator_id": character_id, "user_id": "runtime_user"}
    )
