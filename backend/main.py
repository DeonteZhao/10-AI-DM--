...
def normalize_character_record(raw_character: dict[str, Any]) -> dict[str, Any]:
  ...
  return {
    ...
    "rule_system": "coc",
    "user_id": raw_character.get("user_id"),
...
def require_beta_user_email(authorization: str | None) -> str:
  return require_beta_access_token(authorization)["email"]


def get_character_owner_id(character: dict[str, Any]) -> str | None:
  owner_id = character.get("user_id")
  if isinstance(owner_id, str) and owner_id:
    return owner_id
  return None


def require_owned_character(character_id: str, authorization: str | None) -> tuple[dict[str, Any], str]:
  current_user_email = require_beta_user_email(authorization)
  character = characters.get(character_id)
  if not character:
    raise HTTPException(status_code=404, detail="Character not found")
  owner_id = get_character_owner_id(character)
  if owner_id != current_user_email:
    raise HTTPException(status_code=404, detail="Character not found")
  return character, current_user_email


def get_session_owner_id(session: dict[str, Any]) -> str | None:
  ...
  if not isinstance(investigator_id, str) or not investigator_id:
    return None
  investigator = characters.get(investigator_id)
  if not investigator:
    return None
  return get_character_owner_id(investigator)


def require_owned_session(session_id: str, authorization: str | None) -> tuple[dict[str, Any], str]:
  current_user_email = require_beta_user_email(authorization)
  session = sessions.get(session_id)
  if not session:
    raise HTTPException(status_code=404, detail="Session not found")
  owner_id = get_session_owner_id(session)
  if owner_id != current_user_email:
    raise HTTPException(status_code=404, detail="Session not found")
  session["user_id"] = owner_id
  return session, current_user_email


IP_COUNTRY_CACHE: dict[str, tuple[str | None, float]] = {}


def extract_client_ip(x_forwarded_for: str | None, x_real_ip: str | None) -> str | None:
  candidates: list[str] = []
  if x_forwarded_for:
  ...
  for candidate in candidates:
    try:
      ...
      continue
    if parsed_ip.is_private or parsed_ip.is_loopback or parsed_ip.is_reserved or parsed_ip.is_unspecified:
      continue
    return parsed_ip.compressed
  return None


async def resolve_country_code(
  x_geo_country: str | None,
  x_forwarded_for: str | None,
  x_real_ip: str | None
) -> str | None:
  if x_geo_country:
    return x_geo_country.strip().upper() or None
  client_ip = extract_client_ip(x_forwarded_for, x_real_ip)
  if not client_ip:
    return None
  ...
  try:
    ...
    IP_COUNTRY_CACHE[client_ip] = (None, now)
    return None
  if payload.get("success") is False:
    IP_COUNTRY_CACHE[client_ip] = (None, now)
    return None
  country_code = str(payload.get("country_code") or "").strip().upper() or None
  IP_COUNTRY_CACHE[client_ip] = (country_code, now)
  return country_code


async def resolve_openrouter_model(
  usage: Literal["kp", "import"],
  x_geo_country: str | None,
  x_forwarded_for: str | None,
  x_real_ip: str | None
) -> str:
  country_code = await resolve_country_code(x_geo_country, x_forwarded_for, x_real_ip)
  if country_code == "CN":
    return os.getenv("OPENROUTER_CN_MODEL", "deepseek/deepseek-v3.2-exp").strip() or "deepseek/deepseek-v3.2-exp"
  if usage == "kp":
    return os.getenv("OPENROUTER_KP_MODEL", "").strip() or os.getenv("OPENROUTER_MODEL", "openai/gpt-4o-mini").strip() or "openai/gpt-4o-mini"
  return os.getenv("OPENROUTER_MODEL", "anthropic/claude-3.5-sonnet").strip() or "anthropic/claude-3.5-sonnet"
...
async def run_fallback_import(
  ...
  x_geo_country: str | None = None,
  x_forwarded_for: str | None = None,
  x_real_ip: str | None = None,
) -> tuple[dict[str, Any], dict[str, Any], str]:
  try:
    text = await extract_text_for_import(filename, content, task_id)
    ai_structured = await ai_structure_module(text, x_geo_country, x_forwarded_for, x_real_ip)
  except HTTPException as exc:
    ...
    if task["parser_type"] == "dify":
      ...
      try:
        ...
        raw_output, parsed_output, source_type = await run_fallback_import(
          ...
          task["task_id"],
          dify_error,
          task.get("x_geo_country"),
          task.get("x_forwarded_for"),
          task.get("x_real_ip")
        )
    else:
...
async def ai_structure_module(
  ...
  async with httpx.AsyncClient(timeout=60) as client:
    response = await client.post(
      ...
      json={
        "model": await resolve_openrouter_model("import", x_geo_country, x_forwarded_for, x_real_ip),
        "messages": [
...
async def generate_ai_reply(
  ...
  openrouter_model = await resolve_openrouter_model("kp", x_geo_country, x_forwarded_for, x_real_ip)
...
async def admin_import_module(
  ...
  x_admin_token: str | None = Header(default=None),
  x_geo_country: str | None = Header(default=None),
...
@app.post("/characters")
async def create_character(payload: CharacterCreate, authorization: str | None = Header(default=None)):
  current_user_email = require_beta_user_email(authorization)
...
async def toggle_equip(character_id: str, payload: dict, authorization: str | None = Header(default=None)):
    item_id = payload.get("item_id")
    if not item_id:
        raise HTTPException(status_code=400, detail="Missing item_id")
    
    char, _ = require_owned_character(character_id, authorization)
...
async def add_xp(character_id: str, payload: XPUpdate, authorization: str | None = Header(default=None)):
    char, _ = require_owned_character(character_id, authorization)
    raise HTTPException(status_code=400, detail="当前版本仅支持 COC 调查员链路")

@app.post("/characters/{character_id}/skill-check")
async def skill_check(character_id: str, payload: SkillCheck, authorization: str | None = Header(default=None)):
    char, _ = require_owned_character(character_id, authorization)
...
async def trigger_growth(character_id: str, authorization: str | None = Header(default=None)):
    char, _ = require_owned_character(character_id, authorization)
...
async def list_characters(authorization: str | None = Header(default=None)):
  current_user_email = require_beta_user_email(authorization)
  return {"characters": [item for item in characters.values() if get_character_owner_id(item) == current_user_email]}


@app.post("/sessions")
async def create_session(payload: SessionCreate, authorization: str | None = Header(default=None)):
  current_user_email = require_beta_user_email(authorization)
  try:
  ...
  investigator, _ = require_owned_character(payload.investigator_id, authorization)
...
async def get_session(session_id: str, authorization: str | None = Header(default=None)):
  session, _ = require_owned_session(session_id, authorization)
  investigator_id = session.get("investigator_id")
...
async def get_session_state(session_id: str, authorization: str | None = Header(default=None)):
...
async def resolve_session_check(session_id: str, payload: CheckResolutionRequest, authorization: str | None = Header(default=None)):
  session, _ = require_owned_session(session_id, authorization)
  investigator = characters.get(session["investigator_id"])
  if not investigator:
    raise HTTPException(status_code=404, detail="Investigator not found")
...
async def gm_action_external(payload: ExternalActionRequest, authorization: str | None = Header(default=None)):
  session, _ = require_owned_session(payload.session_id, authorization)
  module = ensure_default_structured(session["scenario_id"])
