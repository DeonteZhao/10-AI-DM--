...
export function BetaAccessGate({ children, initialAccessGranted }: BetaAccessGateProps) {
  ...
  useEffect(() => {
    ...
    const syncAccess = async () => {
      ...
      try {
        ...
        const session = (await response.json()) as BetaAccessSession;
        writeCachedAccess({
          verified: true,
          email: session.email,
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      const payload = (await response.json()) as BetaAccessSendCodeResult | BetaAccessSendCodeError;
      if (!response.ok) {
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      const payload = (await response.json()) as BetaAccessVerifyResult | { detail?: string };
      if (!response.ok) {
  ...
  const handleJoinWaitlist = async () => {
    ...
    try {
      ...
      const payload = (await response.json()) as BetaAccessWaitlistResult | { detail?: string };
      if (!response.ok) {
